...
def xgboost_shap_analysis(X, y):
    ...
    cluster_colors, result_df = perform_clustering(selected_pca_components, k_value, cluster_number, df_features)
...
st.set_page_config(
    page_title="Interpretable feature extraction and clustering framework",
    layout="centered",
    initial_sidebar_state="expanded",
    menu_items={
        'Get Help': 'https://github.com/Tianyi-Shi-Tsukuba',
        'Report a bug': "https://github.com/Tianyi-Shi-Tsukuba",
        'About': "An interpretable feature extraction and clustering tool."
    }
)
st.markdown(
    "<h1 style='text-align: center; color: black;'>Interpretable Feature Extraction and Spectral Clustering</h1>",
    unsafe_allow_html=True
)
data_format_table_feat = pd.DataFrame({
...
st.markdown("<h3 style='text-align: center; color: black;'>Feature Data Table:</h3>", unsafe_allow_html=True)
st.table(data_format_table_feat)
st.markdown("<h6 style='text-align: left; color: black;'>The input data should be a .csv file containing feature values, where rows represent feature names and columns represent sample names.</h8>", unsafe_allow_html=True)
st.markdown("<h3 style='text-align: center; color: black;'>Meta Data:</h3>", unsafe_allow_html=True)
st.table(data_format_table_meta)
st.markdown("<h6 style='text-align: left; color: black;'>The input meta data should be a .csv with sample IDs and corresponding labels. </h8>", unsafe_allow_html=True)

st.write("---")
st.title("Step 1: Data Upload")
uploaded_file = st.file_uploader("Please upload your feature data (csv)", type="csv")
uploaded_file_meta = st.file_uploader("Please upload your meta data (csv)", type="csv")
st.write("---")

if uploaded_file is not None and uploaded_file_meta is not None:
    st.title("Step 2: Preprocessing")
    feat = pd.read_csv(uploaded_file, header=0, index_col=0)
    metadata = pd.read_csv(uploaded_file_meta, header=0)

   
    feat = feat[~feat.index.duplicated()]
    feat = feat.dropna()
    feat = feat[feat.mean(axis=1) >= 1] 
    ...
    feat_T = feat.T
    feat_T.reset_index(inplace=True)
    feat_T = feat_T.rename(columns={"index": "id"})
    merged_data = pd.merge(feat_T, metadata[['id', 'label']], on='id')

    df_features = merged_data

    st.subheader("Uploaded Feature Data")
    ...
    if normalize_option:
        if normalize_option == "Yes (Normalize)":
            norm_method = st.selectbox("Select normalization method", [
                "log2(data+1)", "log2(data)", "log10(data+1)", "log10(data)"
            ])
            X_ = df_features.drop(['id', 'label'], axis=1)
            y = df_features['label']
            z = df_features['id']
        ...
        st.write("---")
        st.title("Step 3: XGBoost + SHAP values")
        st.subheader("XGBoost")
       
        xgboost_shap_analysis(X, y)
    else:
        st.warning("Please select whether to normalize the data in order to proceed to Step 3.")
